$(function(){ // lecture au chargement du document
	...
	function appelleJquery(){ // fonction pour appeler le Jquery 
		...
		$(".delete").on("click", function(e){ 
			...
			if(confirm("Etes vous sur de vouloir supprimer ce membre?")){
				...
				$.ajax({
				url: "http://adventalk.alwaysdata.net/api/delete/"+$(this).attr("Jeremy"),
				type: "post",
		...
		$(".search").on("click", function(e){ 
			...
			$.ajax({
			url: "http://adventalk.alwaysdata.net/api/read/"+$(this).attr("Jeremy"),
			type: "post",
			success: function(reponse){
		...
		$("#formulaire").on("submit", function(e){ 
			...
			var myUrl = (Jeremy != 0)?"http://adventalk.alwaysdata.net/api/update/"+$("#id_membre").val() : "http://adventalk.alwaysdata.net/api/create";
	...
	function totalUser(total = false){
		$.ajax({
			url: "http://adventalk.alwaysdata.net/api/read",
			type: "post",
			success: function(reponse){
